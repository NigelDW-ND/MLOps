	...
	*/
	...
	if($qType == "getComments"){
		...
		while($comRow = sqlsrv_fetch_array($commResults)){
			...
			}elseif($comRow["CommentSubType"] == 'Unclaim'){
				$commentValue = "<b>Query Unclaimed: </b>". $comRow["Comment"];
				$commentData .= 
				'<div id="'. $comRow["UserType"] .'-comment" class="system-comment-data-display">
					<div id="system-Comment-Time">'. $comRow["DateCreated"]->format('d/m/Y H:i') .'</div>	
					<div id="system-Comment-Content">'. $commentValue .'</div>	
				</div>';
			}else{
		...
		$ucReasonSql = "INSERT INTO support.unclaimReasons (unclaimReason, unclaimQueryID, unclaimMentorID) VALUES ('$reasonVal', '$qid', '$mid')";
		$ucCommentSql = "INSERT INTO support.Comments (QueryID, Comment, UserType, CommentNo, CommentType, CommentSubType, UserID)
		VALUES('$qid','$reasonVal','System','$comNewNumVal','Comment','Unclaim','$mid')";
		$ucqQuery = sqlsrv_query($objCon, $ucqSql);
		$ucReasonQuery = sqlsrv_query($objCon, $ucReasonSql);
		$ucCommentQuery = sqlsrv_query($objCon, $ucCommentSql);
		$returnVal = "";
		if($ucqQuery){
		...
		if($ucCommentQuery){
			$returnVal .= " Added Comment for unclaim successfully";
		} else {
			$returnVal .= " Error: " . sqlsrv_errors($objCon);
		}
		echo $returnVal;
		...
		$recepientDetails = '';
		if($userID == 5){
			$recipResult = getParticipantAccount($objCon, $qid);
			$recipRow = sqlsrv_fetch_array($recipResult, SQLSRV_FETCH_ASSOC);
			$recipImagePath = '../site-images/profiles/business/'.$recipRow["ImageURL"];
			$recipImage = '';
			if(file_exists($recipImagePath)){
				$recipImage .= '<img src="../site-images/profiles/business/'.$recipRow["ImageURL"].'" style="width:100px; height:100px; border-radius:50%" alt="fetola">';
			}else{
				$recipImage .= '<img src="../site-images/profiles/business/users.png" style="width:100px; height:100px; border-radius:50%" alt="Default image not found">';
			}
			$recepientDetails = '
			...
			$recipRow = sqlsrv_fetch_array($recipResult, SQLSRV_FETCH_ASSOC);
			$recipImagePath = '../site-images/profiles/mentor/'.$recipRow["ImageURL"];
			$recipImage = '';
			if(file_exists($recipImagePath)){
				$recipImage .= '<img src="../site-images/profiles/mentor/'.$recipRow["ImageURL"].'" style="width:100px; height:100px; border-radius:50%" alt="fetola">';
			}else{
				$recipImage .= '<img src="../site-images/profiles/mentor/users.png" style="width:100px; height:100px; border-radius:50%" alt="Default image not found">';
			}
			$recepientDetails = '
			<div id="recepientProfilePic">
				'.$recipImage.'
