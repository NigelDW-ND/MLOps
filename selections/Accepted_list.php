...
<?php
	...
	$strSQL = " SELECT id_signupdate, title, firstname, lastname, email, phonenumber, entname FROM forms.tb_signup_update WHERE id_signupdate IN (SELECT id_participant_Guid FROM participant.admin) AND entname LIKE '%".$strKeyword."%' ORDER BY addedsql OFFSET (".$page." - 1) * ".$per_page." ROWS FETCH NEXT ".$per_page." ROWS ONLY";
...
while($row = sqlsrv_fetch_array($objQuery, SQLSRV_FETCH_ASSOC))
{
				...
				echo "<tr>
					...
					<? if($GETstatResult['PercentAnsw'] == 100){
						?>
					<button  class="clk_comment_alt" onclick="window.location.href='RoundTable.php?business_key=<? echo $row['id_signupdate'] ?>'">Round Table</button>
						<?
					}else{
					?>
					<button  class="clk_comment" onclick="window.location.href='Particants_interview_page.php?business_key=<? echo $row['id_signupdate'] ?>'">Interview</button>
					<?	
					} 
					 echo "</td>
					</tr>";
