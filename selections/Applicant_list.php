...
<?php
	...
	$strSQL = "SELECT id_signupdate, title, firstname, lastname, email, phonenumber, entname FROM forms.tb_signup_update WHERE id_signupdate NOT IN (SELECT id_participant_Guid FROM participant.admin) AND entname LIKE '%".$strKeyword."%' ORDER BY addedsql; ";
	...
	$strSQL = " SELECT id_signupdate, title, firstname, lastname, email, phonenumber, entname FROM forms.tb_signup_update WHERE id_signupdate NOT IN (SELECT id_participant_Guid FROM participant.admin) AND entname LIKE '%".$strKeyword."%' ORDER BY addedsql OFFSET (".$page." - 1) * ".$per_page." ROWS FETCH NEXT ".$per_page." ROWS ONLY";
...
while($row = sqlsrv_fetch_array($objQuery, SQLSRV_FETCH_ASSOC))
{
				echo "<tr>
					...
					<td>"
					?>
					<button  class="clk_comment" onclick="clk_action('<? echo $row['id_signupdate'] ?>')">Action</button>
...
for($i=1; $i<=$num_pages; $i++){
	...
	{
		echo "<b class='MyButtongreen'>&nbsp; $i &nbsp;</b>";
	}
}
if($page!=$num_pages)
{
